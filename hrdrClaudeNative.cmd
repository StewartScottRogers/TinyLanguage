...
REM  WHAT THIS IS
REM  ------------
REM  A single, self-contained batch file. Drop it in the root of a project, run it
REM  on a bare Windows machine, and in one execution it stands up a working dev
REM  environment for THAT project -- no other files required.
REM
REM  It performs these steps, each a no-op if already satisfied:
REM
REM      [1/6] Ensure git            (installs via winget if missing)
REM      [2/6] Ensure Node.js / npm  (installs via winget if missing)
REM      [3/6] Ensure Claude Code    (installs/updates via npm)
REM      [4/6] Ensure herdr          (installs/updates the terminal multiplexer)
...
REM
REM  WHERE IT WORKS  --  HERE, AND ONLY HERE
REM  ---------------------------------------
REM  The working directory is always the folder this .cmd file lives in (%~dp0),
REM  and its subdirectories. Nothing else. There is deliberately no repo URL, no
REM  clone step and no %USERPROFILE% fallback, so the script cannot reach back at
REM  whatever project it was copied from: move the file into a different project
REM  and it re-anchors there by itself, with no edit required.
REM
REM  That folder becomes the cwd of Claude's pane, and since no --add-dir is
REM  passed, it is also the root of Claude's own workspace.
REM
...
REM  cleanly later. There is no second candidate and no fallback anywhere else on
...
REM -----------------------------------------------------------------------------
REM  [1/6]  Ensure git  (Claude Code leans on it for history, diffs and commits)
REM -----------------------------------------------------------------------------
...
REM -----------------------------------------------------------------------------
REM  [2/6]  Ensure Node.js / npm  (needed to install Claude Code)
REM -----------------------------------------------------------------------------
...
REM -----------------------------------------------------------------------------
REM  [3/6]  Ensure Claude Code  (npm global package), then update it
REM -----------------------------------------------------------------------------
...
echo [3/6] Checking for Claude Code ...
...
REM -----------------------------------------------------------------------------
REM  [4/6]  Ensure herdr, update it, and install its Claude integration hook
REM -----------------------------------------------------------------------------
...
REM -----------------------------------------------------------------------------
REM  [5/6]  Ensure a COMPATIBLE herdr server is running to host the pane
REM -----------------------------------------------------------------------------
...
echo [5/6] Ensuring a herdr server is running ...
set "SRV_RUNNING="
set "SRV_COMPAT="
...
REM  /D pins the new session's working directory to WORK_DIR. Without it the herdr
REM  window would inherit whatever directory this console happened to be sitting in
REM  (which is NOT necessarily this script's folder when it is launched by typing a
REM  path rather than double-clicking), and its default pane would open there.
echo       starting a herdr session ...
start "herdr" /D "%WORK_DIR%" "%HERDR%"
set "STARTED_HERDR=1"
...
REM -----------------------------------------------------------------------------
REM  [6/6]  Launch Claude as a tracked pane inside herdr
REM -----------------------------------------------------------------------------
:server_ready
...
REM  anchored to WORK_DIR -- this script's own folder, the only directory it ever
REM  touches -- with CLAUDE_MODEL forwarded into its environment, and read the new
REM  pane's id out of the JSON response.
set "PANE_ID="
for /f "usebackq delims=" %%P in (`powershell -NoProfile -Command "(& '%HERDR%' tab create --cwd '%WORK_DIR%' --env CLAUDE_MODEL=%CLAUDE_MODEL% --label '%AGENT_LABEL%' --focus | ConvertFrom-Json).result.root_pane.pane_id"`) do set "PANE_ID=%%P"
