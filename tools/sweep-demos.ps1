<#
.SYNOPSIS
  Sweep every TinyLanguage demo (.tlg) through the published exe in file mode and report failures.
...
.DESCRIPTION
  ...
  Exit code: 0 when every demo passes (FAILED=0 TIMEOUT=0), else 1 - so callers can gate on it.
...
Write-Host ("TOTAL={0} FAILED={1} TIMEOUT={2}  (exe {3} MB)" -f $tlgs.Count, $fail.Count, $timeout.Count, $exeSizeMB)
...
if ($fail.Count -gt 0 -or $timeout.Count -gt 0) { exit 1 } else { exit 0 }
