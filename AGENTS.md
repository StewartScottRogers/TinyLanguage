...
**Generated `.cmd`/`.bat` files MUST be written with CRLF line endings.** `cmd.exe` resolves `call :label` / `goto :label` by byte offset and miscounts on LF-only files, so an LF-only installer fails with "The system cannot find the batch label specified" even though the script is correct. This bit `install-vscode-debugger.cmd` in the 2026.05.30 run (Phase 4D). Applies to every generated batch file — the per-demo runners, `run-all-demos.cmd`, and both `install-*-debugger.cmd` installers. (The per-demo/`run-all-demos` finalizer already emits CRLF; the installers must too.)
...
- **`switch` is NO-FALL-THROUGH** (interpreter behavior, Build.Solution.md interpreter coverage "non-matching case body does not execute"): the FIRST matching `case` runs its body and the switch ends; `break` only exits a case body early; `default` runs ONLY when no case matched. Do NOT implement C-style implicit fall-through. (Folded in from the 2026.05.30 run — Phase 4.5 had to fix this on the interpreter side; 4 switch demos depended on it.)
- **Enum members are named INTEGER CONSTANTS.** The interpreter UNWRAPS an `EnumValue` to its underlying integer for arithmetic, comparison, and `switch`/`match` equality, and `Stringify` renders the underlying value. So `Level.Low + Level.High`, `Priority.High == 9`, and `switch Signal.Go { case 1: ... }` all work. (Folded in from the 2026.05.30 run — Phase 4.5 fixed this interpreter-side; 6 enum demos depended on it. Implement it in Phase 3A from the start.)
